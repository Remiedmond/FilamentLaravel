...
<body style="font-family: sans-serif; line-height: 1.6; color: #333;">
    <h2 style="color: #4f46e5;">Bonjour {{ $registration->name }} !</h2>
    <p>Nous vous confirmons votre inscription à l'événement : <strong>{{ $event->title }}</strong>.</p>

    <div style="background: #f3f4f6; padding: 15px; border-radius: 8px;">
        <p><strong> Lieu :</strong> {{ $event->location }}</p>
        <p><strong> Date :</strong> {{ \Carbon\Carbon::parse($event->start_date)->format('d/m/Y H:i') }}</p>
        <p><strong> Accompagnants :</strong> {{ $registration->additional_guests }}</p>
    </div>
<p style="font-size: 0.9em; color: #666;">
    Un imprévu ? Vous pouvez annuler votre réservation et libérer vos places en cliquant sur le bouton ci-dessous :
</p>

<a href="{{ Illuminate\Support\Facades\URL::signedRoute('registration.cancel', ['registration' => $registration->id]) }}"
