...
<div class="min-h-screen bg-[#FDFDFC] pb-20">
    ...
    <div class="max-w-4xl mx-auto px-6 pt-8">
        <a href="<?php echo e(route('events.index')); ?>" class="inline-flex items-center text-sm font-bold uppercase tracking-widest text-[#706f6c] hover:text-[#87a391] transition-colors group">
    ...
    <div class="max-w-4xl mx-auto px-6 mt-8">
        ...
        <div class="flex flex-col gap-4 mb-8">
            <div class="flex items-center gap-3">
                ...
                <span class="text-[#706f6c] text-sm italic"><?php echo e(\Carbon\Carbon::parse($event->getAttribute('date-start'))->translatedFormat('d F Y')); ?></span>
            </div>
        ...
        <div class="relative h-[400px] w-full rounded-[2.5rem] overflow-hidden shadow-2xl mb-12 border border-[#e3e3e0]/50">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($event->cover_image): ?>
                <img src="<?php echo e(Storage::url($event->cover_image)); ?>" alt="<?php echo e($event->title); ?>" class="w-full h-full object-cover">
            <?php else: ?>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div class="bg-white p-8 rounded-[2rem] border border-[#e3e3e0]/50 shadow-sm">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-bold text-[#1b1b18]"><?php echo e($event->location); ?></p>
                            </div>
                        </div>
                        <div class="flex items-start gap-4">
                            <div class="p-2 bg-[#fdfdfc] rounded-lg border border-[#e3e3e0]">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-bold text-[#1b1b18]">Du <?php echo e(\Carbon\Carbon::parse($event->getAttribute('date-start'))->format('d/m/Y à H:i')); ?> Au <?php echo e(\Carbon\Carbon::parse($event->getAttribute('date-end'))->format('d/m/Y à H:i')); ?></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-10 pt-8 border-t border-[#e3e3e0]/50">
                        <div class="flex justify-between items-end mb-2">
                            ...
                            <span class="text-xs font-bold text-[#1b1b18]"><?php echo e($totalInscrits); ?>/<?php echo e($event->max_participants); ?></span>
                        </div>
                        <div class="w-full bg-[#f2f2f0] h-2 rounded-full overflow-hidden">
                            <div class="bg-[#87a391] h-full transition-all duration-1000" style="width: <?php echo e(($totalInscrits / $event->max_participants) * 100); ?>%"></div>
                        </div>
                        <p class="mt-3 text-[11px] italic text-[#706f6c]">Il reste <?php echo e($disponibles); ?> places disponibles</p>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white p-8 md:p-10 rounded-[2.5rem] border border-[#e3e3e0]/50 shadow-sm relative overflow-hidden">
                    ...
                    <form action="<?php echo e(route('events.register', $event->slug)); ?>" method="POST" class="space-y-6 relative">
                        <?php echo csrf_field(); ?>
                        ...
                        <?php $maxAllowed = $event->max_guests_per_registration ?? 0; ?>

                        <div class="pt-4">
                            <div class="p-6 bg-[#fdfdfc] border border-[#e3e3e0] rounded-[2rem] space-y-4">
                                <label class="block text-sm font-bold text-[#1b1b18]">
                                    Accompagnateurs <span class="text-[#87a391] ml-2 font-medium">(Max <?php echo e($maxAllowed); ?>)</span>
                                </label>
                        ...
                        <div id="guests_fields" class="space-y-4">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php for($i = 0; $i < $maxAllowed; $i++): ?>
                                <div id="guest_form_<?php echo e($i); ?>" class="guest-form p-6 border border-[#e3e3e0] rounded-[2rem] bg-white shadow-sm" style="display: none;">
                                    <h4 class="text-xs font-bold uppercase tracking-widest text-[#87a391] mb-4">Accompagnateur n°<?php echo e($i + 1); ?></h4>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <input type="text" name="guests[<?php echo e($i); ?>][firstname]" placeholder="Prénom" class="px-4 py-3 bg-[#fdfdfc] border border-[#e3e3e0] rounded-xl outline-none">
                                        <input type="text" name="guests[<?php echo e($i); ?>][lastname]" placeholder="Nom" class="px-4 py-3 bg-[#fdfdfc] border border-[#e3e3e0] rounded-xl outline-none">
                                        <input type="email" name="guests[<?php echo e($i); ?>][email]" placeholder="Email (optionnel)" class="md:col-span-2 px-4 py-3 bg-[#fdfdfc] border border-[#e3e3e0] rounded-xl outline-none">
                                        <textarea name="guests[<?php echo e($i); ?>][allergies]" placeholder="Allergies (optionnel)" class="md:col-span-2 px-4 py-3 bg-[#fdfdfc] border border-[#e3e3e0] rounded-xl outline-none"></textarea>
                                    </div>
                                </div>
                            <?php endfor; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </div>
